---
title: "Descriptive analysis on the imputed NKR dataset"
...
# Aim(s)
1. To produce Table 1’s for the imputed NKR dataset
...
# Additional info: observed 5-year mortalities
```{r Obsmort_5yrs}
dat$wt <- 1/10 #rescale counts

obs <- survfit(Surv(time, os) ~ 1, weight = wt, data = dat)
summary(obs, times = 5, extend = T)$n.event #97 events at 5 years
round(summary(obs, times = 5, extend = T)$n.event * 100/3323, 1) #2.9% of total (not at risk)

obs_bc <- survfit(Surv(time, eventbc) ~ 1, weight = wt, data = dat)
summary(obs_bc, times = 5, extend = T)$n.event #43 events at 5 years
round(summary(obs_bc, times = 5, extend = T)$n.event * 100/3323, 1) #1.3% of total

obs_oth <- survfit(Surv(time, eventoth) ~ 1, weight = wt, data = dat)
summary(obs_oth, times = 5, extend = T)$n.event #54 events at 5 years
round(summary(obs_oth, times = 5, extend = T)$n.event * 100/3323, 1) #1.6% of total
```
